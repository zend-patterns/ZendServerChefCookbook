...
def load_current_resource
  @current_resource = Chef::Resource::ZendserverPear.new(@new_resource.name)
  @current_resource.package_name(@new_resource.package_name)
  @bin = '/usr/local/zend/bin/pear'
  if pecl?
    Chef::Log.debug("#{@new_resource} smells like a pecl...installing package in Pecl mode.")
    @bin = '/usr/local/zend/bin/pecl'
  end
...
# Assuming running Ruby 1.9, since .chr resulted in undefined method
  ext_prefix << ::File::SEPARATOR if ext_prefix[-1] != ::File::SEPARATOR
  ...
  template "#{node['zendserver']['ext_conf_dir']}/#{name}.ini" do
...
def pecl?
  @pecl ||= begin
    ...
    search_cmd = "/usr/local/zend/bin/pear -d preferred_state=#{can_haz(@new_resource, "preferred_state")} search#{expand_channel(can_haz(@new_resource, "channel"))} #{@new_resource.package_name}"
    Chef::Log.info(search_cmd)
    if shell_out(search_cmd).stdout.split("\n").find { |line| line =~ /^#{@new_resource.package_name}\s+#{haz_ver(@new_resource, "version")}/ }
      ...
      search_cmd = "/usr/local/zend/bin/pecl -d preferred_state=#{can_haz(@new_resource, "preferred_state")} search#{expand_channel(can_haz(@new_resource, "channel"))} #{@new_resource.package_name}"
      Chef::Log.info(search_cmd)
      if shell_out(search_cmd).stdout.split("\n").find { |line| line =~ /^#{@new_resource.package_name}\s+#{haz_ver(@new_resource, "version")}/ }
        true
      else
